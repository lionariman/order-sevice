...
func genOrder() *intl.Order {
	...
	return &intl.Order{
		...
		Delivery: intl.Delivery{
			Name:    pick([]string{"Nikolay Petrov", "Pierre Dubois", "Hans Schneider", "William Turner"}),
			Phone:   pick([]string{"+7", "+49", "+33", "+44"}) + randomDigits(10),
			Zip:     pick([]string{"101000", "190000", "2639809", "700670"}),
			City:    pick([]string{"Moscow", "Paris", "Berlin", "London"}),
			Address: pick([]string{"Rue Rouget-de-L'Isle", "Ploshad Mira 15", "Unter den Linden 5", "2 Oxford St."}),
			Region:  pick([]string{"Russia", "Germany", "United Kingdom", "France"}),
			Email:   strings.ToLower(randHex(4)) + "@example.com",
		},
		Payment: intl.Payment{
			Transaction:  orderUID, // как в примере
			RequestID:    "",
			Currency:     pick([]string{"RUB", "USD", "EUR", "JPY"}),
			Provider:     "wbpay",
			Amount:       amount,
			PaymentDT:    now.Unix(),
